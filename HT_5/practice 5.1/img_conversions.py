...
class ImageConverter:
    @staticmethod
    def mono_to_mono_stat(
        m: MonoImage, target_mean: float = 127.0, target_std: float = 64.0
    ) -> MonoImage:
    ...
    @staticmethod
    def color_to_color_stat(
        c: ColorImage,
        target_means=(127.0, 127.0, 127.0),
        target_stds=(64.0, 64.0, 64.0),
    ) -> ColorImage:
        ...
        for ch in range(3):
            ...
            out[..., ch] = np.clip(
                (chdat - mean) / std * target_stds[ch] + target_means[ch], 0, 255
            )
    ...
    def color_to_bin(c: ColorImage, threshold: int | None = None) -> BinaryImage:
        return ImageConverter.mono_to_bin(
            ImageConverter.color_to_mono(c), threshold=threshold
        )
...
# import numpy as np
# from img_conversions import *

# x = np.tile(np.linspace(0,255,64,dtype=np.uint8),(64,1))
# m = MonoImage(x)
# c = ColorImage(np.stack([x, 255-x, x//2], axis=2))
# b = ImageConverter.mono_to_bin(m)

# m2 = ImageConverter.mono_to_mono_stat(m)
# c2 = ImageConverter.color_to_color_stat(c)
# g  = ImageConverter.color_to_mono(c)
# col= ImageConverter.mono_to_color(m)
# d  = ImageConverter.bin_to_mono_distance(b)
# cb = ImageConverter.color_to_bin(c)
# bc = ImageConverter.bin_to_color(b)
